...
int jailhouse_cmd_cell_create(struct jailhouse_cell_create __user *arg)
{
	...
	pr_info("[wheatfox] (jailhouse_cmd_cell_create) start\n");
	...
	pr_info("[wheatfox] (jailhouse_cmd_cell_create) Created cell \"%s\"\n", cell->name);
...
int jailhouse_cmd_cell_load(struct jailhouse_cell_load __user *arg)
{
	...
	unsigned int n;
	int err;
	pr_info("[wheatfox] (jailhouse_cmd_cell_load) start\n");
	...
	if (err)
		return err;
	pr_info("[wheatfox] (jailhouse_cmd_cell_load) cell_management_prologue(&cell_load.cell_id, &cell) done\n");

	err = jailhouse_call_arg1(JAILHOUSE_HC_CELL_SET_LOADABLE, cell->id);
	if (err)
		goto unlock_out;

	pr_info("[wheatfox] (jailhouse_cmd_cell_load) jailhouse_call_arg1(JAILHOUSE_HC_CELL_SET_LOADABLE, cell->id) done\n");
	pr_info("[wheatfox] (jailhouse_cmd_cell_load) cell_load.num_preload_images = %d\n", cell_load.num_preload_images);
	for (n = cell_load.num_preload_images; n > 0; n--, image++) {
		pr_info("[wheatfox] (jailhouse_cmd_cell_load) load_image(cell, image) start, n = %d\n", n);
		err = load_image(cell, image);
		if (err)
			break;
		pr_info("[wheatfox] (jailhouse_cmd_cell_load) load_image(cell, image) done, n = %d\n", n);
		
	}

unlock_out:
	mutex_unlock(&jailhouse_lock);
	pr_info("[wheatfox] (jailhouse_cmd_cell_load) unlock_out done, exiting jailhouse_cmd_cell_load\n");
	return err;
}
...
{	
	...
	pr_info("[wheatfox] (jailhouse_cmd_cell_start) start\n");
	
	if (copy_from_user(&cell_id, arg, sizeof(cell_id)))
		return -EFAULT;

	pr_info("[wheatfox] (jailhouse_cmd_cell_start) copy_from_user(&cell_id, arg, sizeof(cell_id)) done\n");

	err = cell_management_prologue(&cell_id, &cell);
	if (err)
		return err;

	pr_info("[wheatfox] (jailhouse_cmd_cell_start) cell_management_prologue(&cell_id, &cell) done\n");
