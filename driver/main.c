...
static int jailhouse_cmd_enable(struct jailhouse_system __user *arg)
{
	...
	pr_info("[wheatfox] Loading hypervisor image %s\n", fw_name);
	// the first arg is for storing the firmware image
	// the second arg is for the name of the firmware image
	// the third arg is for the device that requests the firmware
	...
	pr_info("[wheatfox] The Jailhouse is opening.\n");
...
{
	long err;
	pr_info("[wheatfox] (jailhouse_ioctl) ioctl=0x%x arg=%ld\n", ioctl, arg);
	switch (ioctl) {
	case JAILHOUSE_ENABLE:
		pr_info("[wheatfox] (jailhouse_ioctl) JAILHOUSE_ENABLE\n");	
