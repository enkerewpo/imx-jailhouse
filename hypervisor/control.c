...
{
	printk("[wheatfox] in resume_cpu, cpu_id = %d\n", cpu_id);
	struct public_per_cpu *target_data = public_per_cpu(cpu_id);
	...
	spin_unlock(&target_data->control_lock);
	printk("[wheatfox] in resume_cpu, target_data->cpu_suspended = %d\n", target_data->cpu_suspended);
}
...
static int cell_start(struct per_cpu *cpu_data, unsigned long id)
{
	...
	printk("[wheatfox] in cell_start, cell->loadable = %d\n", cell->loadable);
	if (cell->loadable) {
		printk("[wheatfox] cell is loadable\n");
		...
		cell->loadable = false;
		printk("[wheatfox] finished unmap all loadable memory regions from the root cell\n");
	}
	...
	printk("[wheatfox] finished pci_cell_reset and arch_cell_reset\n");
	...
	printk("[wheatfox] finished arch_reset_cpu\n");

	printk("[wheatfox] Started cell \"%s\"\n", cell->config->name);
...
static int cell_set_loadable(struct per_cpu *cpu_data, unsigned long id)
{
	...
	printk("[wheatfox] cell_management_prologue finished\n");
	/*
	...
	printk("[wheatfox] finished arch_park_cpu\n");
	printk("[wheatfox] cell->loadable = %d\n", cell->loadable);

	if (cell->loadable)
		goto out_resume;

	printk("[wheatfox] not jumped to out_resume\n");
...
{	
	...
	case JAILHOUSE_HC_CELL_CREATE:
		printk("[wheatfox] in hypercall, JAILHOUSE_HC_CELL_CREATE\n");
		return cell_create(cpu_data, arg1);
	case JAILHOUSE_HC_CELL_START:
		printk("[wheatfox] in hypercall, JAILHOUSE_HC_CELL_START\n");
		return cell_start(cpu_data, arg1);
	case JAILHOUSE_HC_CELL_SET_LOADABLE:
		printk("[wheatfox] in hypercall, JAILHOUSE_HC_CELL_SET_LOADABLE\n");
